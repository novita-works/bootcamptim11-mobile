...
${CONFIRM_ORDER}                //android.widget.Button[@resource-id="com.example.myapplication:id/confirm_order"]
${ENDDATE_09}                   //android.view.View[@content-desc="09 August 2019"]
${SUCCESS_BOOK}                 //android.widget.CheckedTextView[@resource-id="com.example.myapplication:id/checkedTextView"]
${BUTTON_ENDDATE}               '//android.widget.Button[@resource-id="android:id/button1"]'

${DEPARTURE_CITY}               Chicago
${DEPARTURE_CITY_1}             New York
${ARRIVAL_CITY}                 London
${ARRIVAL_CITY1}                Paris
${ARRIVAL_CITY2}                Ottawa
...
User Should Be Able To Book Flight One Way Valid
    Click Button Booking
    Select One Way Trip
    Select Departure City    ${DEPARTURE_CITY1}
    Select Arrival City    ${ARRIVAL_CITY2}
    Select Flight Class    ${FIRST_CLASS}
    Select Flight Only
    Select Departure Date    ${FUTURE_DATE}
    Click OK Date Button
    Select Departure Date    ${PAST_DATE_VALID}
    Select End Date    ${ENDDATE_09}
    Click OK Date Button
    Book Flight
    Confirm Order
    Verify Success Book

User Should Be Able To Book Flight Round Trip Valid
    Click Button Booking
    Select One Way Trip
    Select Departure City    ${DEPARTURE_CITY1}
    Select Arrival City    ${ARRIVAL_CITY2}
...
Select OK Button End Date
    Wait Until Element Is Visible    locator=locator=${BUTTON_ENDDATE}    timeout=10s
    Click Element    locator=${BUTTON_ENDDATE}
    Sleep    2s
...
Select Round Trip
    Wait Until Element Is Visible    locator=${TAB_ROUND_TRIP}    timeout=15s
    Click Element    locator=${TAB_ROUND_TRIP}
    Sleep    2s
...
Select Flight Class
    ...
    ${class_locator}=    Set Variable
    ...    //android.widget.TextView[@resource-id="android:id/text1" and @text="${class}"]
...
Select End Date
    [Arguments]    ${days_from_today}
    # Open date picker
    Wait Until Element Is Visible    locator=${ENDDATE_09}    timeout=15s
    Click Element    locator=${ENDDATE_09}
    Sleep    2s
...
Confirm Order
    Wait Until Element Is Visible    locator=${CONFIRM_ORDER}
    Click Element    locator=${CONFIRM_ORDER}
...
Verify Success Book
    Wait Until Element Is Visible    locator=${SUCCESS_BOOK}
...
Verify Test Does Not Route To Certain Page
    ...
    ...    locator=${unexpected_page_locator}
    ...    timeout=5s

    # Fail if the page is found
    IF    '${status}' == 'PASS'    Fail    Unexpected page was routed
...
Test Should Not Route To Booking Screen
    ...
    IF    ${book_page_visible}
        Fail    Test routed to booking screen when it should not have
    END
